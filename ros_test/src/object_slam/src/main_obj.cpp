...
bool whether_use_ground_value;
bool save_results_to_txt;
cv::Mat_<float> matx_to3d_, maty_to3d_;
double magic_number;
bool whether_use_3d_IoU;

// added benchun20200918 calculate distance between cubes only consider 8 corners
double mse_calculation(const cuboid *sample_obj, const cuboid *ground_obj)
{
    ...
    double mse_error = 0;
    for (unsigned corner_idx = 0; corner_idx < ground_3d_corners.cols(); corner_idx++)
    {
    ...
    mse_error = mse_error / ground_3d_corners.cols();
    return mse_error;
}
...
visualization_msgs::MarkerArray cuboids_to_marker(object_landmark* obj_landmark, Vector3d rgbcolor) 
{
    ...
    Eigen::MatrixXd cube_corners = cube_opti.compute3D_BoxCorner();
		// std::cout << "cube_opti" << cube_opti.toMinimalVector().transpose() << std::endl;
		// std::cout << "cube_corners" << cube_corners << std::endl;
		       ...
		       std::vector<object_landmark*> all_frame_rawcubes, Eigen::MatrixXd& truth_frame_poses)
{
	...
	for (int i=0;i<total_frame_number;i++)
	{
	    ...
	    resultsFile << all_frames[i]->cam_pose_Twc.toXYZPRYVector().transpose()<<"\n";
	}
	resultsFile.close();
	...
	if (frame_number<total_frame_number)
	{
	    ...
	    if (all_frame_rawcubes.size()>0 && all_frame_rawcubes[frame_number]!=nullptr)
			...
			// pub_frame_raw_cube.publish(cuboids_to_marker(all_frame_rawcubes[magic_number],Vector3d(0,0,1)));
...
{  
    ...
    detect_cuboid_obj.max_cuboid_num = magic_number;
    ...
    for (int frame_index=0;frame_index<total_frame_number;frame_index++)
    {
	  ...
	  else
	  {
		...
		if (frame_index>1)  // from third frame, use constant motion model to initialize camera.
		{
		    ...
		    odom_val = prev_pose_Tcw*prev_prev_pose_Tcw.inverse();
				// std::cout << frame_index-1 << prev_pose_Tcw.inverse().toXYZPRYVector().transpose()<< std::endl;
		}
		curr_cam_pose_Twc = (odom_val*prev_pose_Tcw).inverse();
	  }
		
		// std::cout << frame_index << curr_cam_pose_Twc.toXYZPRYVector().transpose()<< std::endl;
	  ...
	  if (online_detect_mode)
	  {
		...
		if (has_detected_cuboid)  // prepare object measurement
		{
			...
			// added benchun 20200917
			if(whether_use_3d_IoU)
			{
				detected_cube->skew_ratio = 0;
				int final_cube_idx = 0; 
				for (int cube_idx = 0; cube_idx < frames_cuboids[0].size(); cube_idx++)
				{  
					cuboid *sample_obj = frames_cuboids[0][cube_idx];
					Eigen::Matrix<double, 1, 8> cube_ground_truth; // num, xyz, roty, dimension
					cube_ground_truth << 0, -1.52988, 0.456476, 0.281575,  3.10232, 0.396318, 0.222742, 0.272709;
					...
					sample_obj->skew_ratio = 0;
					sample_obj->skew_ratio = box3d_iou(sample_obj, ground_obj);
					// sample_obj->skew_ratio = mse_calculation(sample_obj, ground_obj);
					if(sample_obj->skew_ratio > detected_cube->skew_ratio)
					{detected_cube = sample_obj;
					final_cube_idx = cube_idx;
					// std::cout << frame_index << " " << cube_idx <<"skew: " << sample_obj->skew_ratio<< std::endl;
					// select_cube->print_cuboid();
					}
				}
				std::cout << frame_index << " final: " << final_cube_idx <<" 3d_iou: " << detected_cube->skew_ratio
										<< " cube: "<< detected_cube->pos.transpose()<< std::endl;
			} // loop use 3d iou
		...
		} // has_detected_cuboid loop
	  }	// online_mode loop 
	  else
	  {
		// // // read ground-based value to local value 
		if(whether_use_ground_value)
		{
			...
			if (has_detected_cuboid)  // prepare object measurement   not all frame has observation!!
			{
				...
				Vector9d cube_pose;cube_pose<<measure_data(1),measure_data(2),measure_data(3),
						measure_data(4),measure_data(5),measure_data(6),
						measure_data(7),measure_data(8),measure_data(9);  // xyz roll pitch yaw scale
				cube_ground_value.fromMinimalVector(cube_pose);
				Eigen::VectorXd cam_pose_vec = init_frame_poses.row(offline_cube_obs_row_id);
				// g2o::SE3Quat cam_val_Twc(cam_pose_vec.segment<7>(1)); // time x y z qx qy qz qw
				g2o::SE3Quat cam_val_Twc;			cam_val_Twc.fromVector(cam_pose_vec.segment<7>(1)); // not normalized
				// std::cout << "cam_pose_vec: " << cam_pose_vec.transpose() << std::endl;
				// std::cout << "cam_val_Twc: \n" << cam_val_Twc.to_homogeneous_matrix() << std::endl;

				cube_local_meas = cube_ground_value.transform_to(cam_val_Twc); // measurement is in local camera frame
				std::cout << "cube_ground_value: " << cube_ground_value.toMinimalVector().transpose() << std::endl;
				// std::cout << "cube_local_meas: " << cube_local_meas.toMinimalVector().transpose() << std::endl;
				// std::cout << offline_cube_obs_row_id << " cube_local_meas: " << cube_local_meas.toMinimalVector().transpose() << std::endl;
		...
		else	// // read local values 
		{
			int cube_obs_frame_id = offline_pred_frame_objects(offline_cube_obs_row_id,0);
			has_detected_cuboid = cube_obs_frame_id==frame_index;
			if (has_detected_cuboid)  // prepare object measurement   not all frame has observation!!
			{
			VectorXd measure_data = offline_pred_frame_objects.row(offline_cube_obs_row_id).tail<9>();
			Vector9d cube_pose;
			cube_pose<<measure_data(0),measure_data(1),measure_data(2),measure_data(3),measure_data(4),
								measure_data(5),measure_data(6),measure_data(7),measure_data(8);  // xyz roll pitch yaw scale
			cube_local_meas.fromMinimalVector(cube_pose);
			Eigen::VectorXd cam_pose_vec = init_frame_poses.row(offline_cube_obs_row_id);
			g2o::SE3Quat cam_val_Twc;			
			cam_val_Twc.fromVector(cam_pose_vec.segment<7>(1)); // not normalized
			// std::cout << "cam_val_Twc: \n" << cam_val_Twc.to_homogeneous_matrix() << std::endl;
			// std::cout << "cube_local_meas: " << cube_local_meas.toMinimalVector().transpose() << std::endl;
	  ...
	  if (all_frames[frame_index]->observed_cuboids.size()>0)
	  {
		...
		g2o::cuboid cube_temp = cube_pose_raw_detected_history[frame_index]->cube_vertex->estimate();
...
int main(int argc,char* argv[])
{
    ...
    nh.param ("/online_detect_mode", online_detect_mode, false);
    nh.param ("/whether_use_ground_value", whether_use_ground_value, false);
    nh.param ("/save_results_to_txt", save_results_to_txt, false);
    nh.param ("/whether_use_3d_IoU", whether_use_3d_IoU, false);
    nh.param ("/magic_number", magic_number, 0.0);
	...
	// 100 is some large row number, each row should equal data numbers
    Eigen::MatrixXd pred_frame_objects(100,10);  // index, xyz, roll, pitch, yaw, dimension
    ...
    std::cout<<"read data size:  "<<pred_frame_objects.rows()<<"  "<<init_frame_poses.rows()
			<<"  "<<truth_frame_poses.rows()<<" "<<truth_objects.rows()<<std::endl;
